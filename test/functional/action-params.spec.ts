...
describe(``, () => {
    ...
    beforeAll(done => {
        ...
        class UserActionParamsController {
            ...
            getPhotosWithMultipleStringValuesRequired(@QueryParam("multipleStringValues", { required: true }) values: string[]): string {
        ...
        class SecondUserActionParamsController {
            ...
            @Post("/users")
            postUser(@BodyParam("name") name: string, @BodyParam("age") age: number, @BodyParam("isActive") isActive: boolean): null {
                bodyParamName = name;
                bodyParamAge = age;
  ...
  */
    ...
    it("@QueryParams should give a proper values from request's query parameters", async () => {
        ...
        expect(queryParams1.multipleDateValues).toEqual([new Date("2017-02-01T00:00:00Z"), new Date("2017-03-01T00:00:00Z")]);
    });
    ...
    it("@UploadedFile with @Body should return both the file and the body", async () => {
        ...
        expect(response.status).toEqual(HttpStatusCodes.OK);
        expect(response.headers["content-type"]).toEqual("text/html; charset=utf-8");
        expect(response.data).toEqual(`<html><body>sample-text-file.txt - {"anotherField":"hello","andAnother":"world"}</body></html>`);
    });
